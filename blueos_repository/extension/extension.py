...
from extension.models import ExtensionMetadata, ExtensionVersion, ExtensionType, Image, Platform
# Extra
import json5
...
class Extension:
    ...
    versions: Dict[str, ExtensionVersion] = {}
    ...
    async def __create_version_from_tag_blob(self, version_tag: Tag, blob: Blob) -> ExtensionVersion:
        ...
        if readme is not None:
            readme = readme.replace(r"{tag_name}", version_tag.name)
            try:
    ...
    async def __process_tag_version(self, tag: Tag) -> None:
        ...
        try:
            ...
            print(f" INVALID-EXTENSION-TAG - Failed to generate version for {tag_name}, in extension {self.metadata.identifier}, error in key: {error}")
        except Exception as error:
            print(f" INVALID-EXTENSION-TAG - Failed to generate version for {tag_name}, in extension {self.metadata.identifier}, exception: {error}")
    ...
    async def inflate(self) -> None:
        ...
        try:
            ...
            print(f" INVALID-EXTENSION - Failed to fetch tags for {self.metadata.identifier}, exception: {error}")
            return
