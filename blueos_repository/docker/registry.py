...
from docker.auth import DockerAuthAPI, AuthToken
# Models
from docker.models.blob import Blob
from docker.models.manifest import ImageManifest, ManifestList, ManifestFetch
# Extra
...
class DockerRegistry:
    ...
    async def get_manifest(self, tag_or_digest: str) -> ManifestFetch:
        ...
        header = {
            "Authorization": f"Bearer {self.__token.token}",
        ...
        if "config" in manifest:
            return ManifestFetch(
                is_image_manifest=True,
                manifest=fromdict(ImageManifest, manifest)
            )

        return ManifestFetch(
            is_image_manifest=False,
            manifest=fromdict(ManifestList, manifest)
        )
