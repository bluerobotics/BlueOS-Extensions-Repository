...
from auth import DockerAuthAPI, AuthToken
# Models
from models.blob import Blob
from models.manifest import ImageManifest, ManifestList, ManifestFetch
...
class DockerRegistry:
    ...
    async def get(self, route: str, max_retries: Optional[int] = None, **kwargs) -> Any:
        """
    ...
    async def get_manifest(self, tag_or_digest: str) -> ManifestFetch:
        ...
        manifest = await self.api.get(route, headers=header)

        if "config" in manifest:
            return ManifestFetch(is_image_manifest=True, manifest=ImageManifest(**manifest))

        return ManifestFetch(is_image_manifest=False, manifest=ManifestList(**manifest))
    ...
    async def get_manifest_blob(self, digest: str) -> Blob:
        ...
        blob = await self.api.get(route, headers=header)
